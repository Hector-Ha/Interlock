...
async function checkUserBalances(email: string) {
  ...
  for (const bank of user.banks) {
    ...
    console.log(`     Dwolla Funding URL: ${bank.dwollaFundingUrl || "N/A"}`);
    ...
    try {
      ...
      for (const acc of accounts) {
        ...
        console.log(
          `       Balance: $${balance} (Available: ${acc.balances.available}, Current: ${acc.balances.current})`,
        );
      }
...
async function main() {
  ...
  console.log(`\n================================`);
  console.log(`GRAND TOTAL (All Users): $${grandTotal}`);
  console.log(`================================`);
}
