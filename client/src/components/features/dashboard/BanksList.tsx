    ...
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {banks.map((bank) => (
        <Link key={bank.id} href={`/banks/${bank.id}`}>
          <div className="group relative flex flex-col justify-between rounded-2xl border border-border/50 bg-card p-6 shadow-md transition-all duration-300 hover:shadow-xl hover:shadow-brand-main/5 hover:-translate-y-1 cursor-pointer h-full">
              ...
              <span
                className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider ${
                  bank.status === "ACTIVE"
                    ...
                    : "bg-warning-surface text-warning-main"
                }`}
              >
            ...
            <div className="mt-2">
              <h3 className="text-lg font-bold text-foreground truncate group-hover:text-brand-main transition-colors">
